...
- **Klosters** → `4087` and **Rapperswil-Jona** → `900040764`. Every other entry in the CSV is a
  ...
  `dest_type` (e.g. `district`/`region`) rather than `city`. Flagged, but **decision made: keep both as
  `dest_type: city` as given** — not re-verified before this implementation. The schema stores
  `destType` per-row (not as a single global constant), so correcting either one later, if it turns out
  wrong, is a one-row data edit, not a schema change.
...
[
  ...
  { "identifier": "27240e44-1e3d-4288-9728-df613a4e6bbf", "name": "Klosters", "category": "village", "destId": "4087", "destType": "city" },
...
`destType` defaults to `"city"` for every row for now, **except Klosters and Rapperswil-Jona should be
re-verified and corrected before the seed is imported** (see "Mapping data" above).
