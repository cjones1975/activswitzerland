...
- Committed on `feature/mobile-drawer-bottom-sheet`, later merged into `desktop-redesign`
...
### 2026-09-12 — Hikes Admin Specced & Implemented

- Branch: `feature/hikes-admin`
- Specced then implemented in the same pass: a `hikes` MongoDB collection for custom GPX-sourced routes, merged into the existing `/api/v1/hikes`/`/api/v1/hikes/search` responses alongside live `ch.astra.wanderland` routes, plus a single-admin-only `/admin` section (gated by the same `CURATED_TRIPS_USER_ID` account already used for curated trips) to add/rename/delete them without touching code per hike
- Created `context/features/hikes-admin-spec.md`
- Preceded by a design-canvas mockup (static, 3 artboards: hikes list, add-hike drawer with GPX preview, and an illustrative future "Users" section) to align on the admin shell's shape and extensibility before speccing
- Backend: new `Hike` model (single-stage, `category: 'local'`, `source: 'loisirs.ch'` by default); `utils/customHikes.js` (`parseGpx` via `fast-xml-parser` — computes distance/ascent/descent/elevation directly from the GPX's own `<ele>` values rather than calling geo.admin.ch's DEM service; `generateRouteNumber`; `fetchCustomHikesNear`/`searchCustomHikes` merged into `getHikes`/`getHikesSearch`); `schweizMobilRoutes.js` gained an exported `linesDistanceMeters` and new `reprojectToLv95`, plus a `source: 'SwitzerlandMobility'` field on ASTRA routes so both sources carry the same attribution field; extracted the existing curated-trip account check (`controllers/trips.js`'s old `isCuratedAccount`) into a shared `utils/adminAccount.js#isAdminAccount`, reused by new `middleware/requireAdmin.js` and by `/auth/me`'s new `isAdmin` flag; new `controllers/adminHikes.js`/`routes/adminHikes.js` (list/preview/create/rename/delete — delete cascades a `$pull` of the hike out of any saved trip's `activities`)
- Frontend: new `/admin` route (`AdminShell` sidebar + `AdminHikes` list/add-drawer), `core/guards/admin.ts`, `AdminHikesService`; the add-hike drawer parses the GPX client-side (`File.text()`, no multipart upload) and shows a live `MapComponent` preview + computed stats before saving; desktop "use mobile" notice bypassed by path (`/admin`) in both `breakpoint.ts` and `index.html`'s no-flash script; `TrailRoute` gained a `source` field
- Two bugs found via live testing, both fixed on the same branch: (1) `/admin` fell through to `RenderMode.Server` in `app.routes.server.ts`, so `adminGuard`'s `isLoggedIn()` check ran during SSR — no `localStorage` on the server — and bounced even a logged-in admin home before the client's real token was ever read; fixed by adding `{ path: 'admin/**', renderMode: RenderMode.Client }`, mirroring the existing `:lang/auth/**` rule and its identical reasoning; (2) `AdminHikes`'s `canSave` computed read `nameModel` as a plain field bound via `[(ngModel)]`, which a `computed()` never tracks as a dependency — the Save button could stay stuck disabled if the name was typed after picking the GPX file, since the computed only re-evaluated (and re-read the name fresh) when the `preview` signal itself later changed; fixed by making `nameModel` a real signal (`[ngModel]`/`(ngModelChange)` instead of `[(ngModel)]`)
- Also sorted `## History` below into correct earliest-to-latest order — it had drifted badly out of order over time (a 2026-09-11 entry sat at the very top, and a large 2026-06/07 block ran partly backwards); verified content-preserving via a word-multiset checksum before/after
- Committed as `bd407c0` (feature) + `151bc4c` (merge into `main`), pushed to `origin/main`

### 2026-09-16 — Garmin GPX Export Fix + Favicon/PWA Icon Refresh

- `utils/schweizMobilRoutes.js#buildGpx()`: now emits a single `<trk>` containing a single `<trkseg>` (previously one `<trkseg>` per line/stage segment) — Garmin Connect's course importer only reliably reads the first `<trkseg>` (and the first `<trk>`) of a GPX file, silently dropping the rest. Found via live testing: a real admin-uploaded custom hike (`L'Isle - Montricher`, converted from a Google Maps KML via GPS Visualizer, stored as 4 line segments/964 points) only showed its first leg once imported into Garmin Connect. Fix verified directly against the running backend — POSTed the hike's real stored `stages` to `/api/v1/hikes/gpx` and confirmed the output flattened to 1 `<trk>`/1 `<trkseg>`/964 `<trkpt>`, in order. `buildGpx` is shared by `hikingRoutes.js` and `bikeRoutes.js`, so this fixes GPX downloads for ASTRA routes, bike routes, and custom admin-added hikes alike; internal map-rendering geometry (kept as separate lines for gap display on our own map) is unaffected. First live test hit stale code because the backend process hadn't restarted since the fix landed; **confirmed working end-to-end in Garmin Connect** after a restart and re-test
- Favicon/PWA icon set replaced (outside this session, picked up while reviewing the working tree): old `manifest.webmanifest` + the `android-icon-*`/`apple-icon-*`/`apple-icon-precomposed.png` set and `BingSiteAuth.xml` removed; new `site.webmanifest`, `favicon.svg`, `favicon-96x96.png`, `apple-touch-icon.png`, `web-app-manifest-192x192.png`/`web-app-manifest-512x512.png` added, `favicon.ico` updated, and `index.html`'s `<link>`/`<meta>` tags updated to match — supersedes the icon set added in the (separately committed, now superseded) "feat: add PWA manifest and touch icons" commit
- `infra/.env.prod.example` also had local-only changes at commit time, excluded as always ([[project-env-prod-example-uncommitted]])

### 2026-09-16 — GPX Segment Order Fix + Start/End Trail Markers

- Follow-on to the same-day Garmin GPX export fix above, found via further live testing: flattening into a single `<trkseg>` fixed "only shows the first leg" but introduced a new bug on multi-segment uploads - segments were flattened in raw storage order, which isn't necessarily their true geographic order, producing multi-km straight-line "teleports" across the map (confirmed on a real admin-uploaded hike, `Col de la Givrine - St. Cergue`, GPS-Visualizer-converted from a Google Maps KML, stored as 4 segments of 39/214/199/248 points). Diagnosed by measuring endpoint-to-endpoint distances between segments directly: the true order was the exact reverse of storage order, connecting to within 0-3m - genuinely one continuous trail, just split and stored out of order
- `utils/schweizMobilRoutes.js`: new exported `stitchLines()` - greedy nearest-endpoint chaining (checks both ends of the growing chain against both orientations of every unused segment) that reorders and reverses segments into a sensible sequence; not a global optimum (NP-hard in general) but correct for the common case of a handful of out-of-order segments. Verified against the real 4-segment hike: reconstructs the correct 248→199→214→39 order, preserves all 700 points, cuts the worst remaining gap from ~4.9km to 86m
- Applied **per stage**, not by pooling every stage's lines together - a multi-day route's stage order (day 1, day 2, ...) is already authoritative and often has a deliberate large gap between days, which a global nearest-neighbor pass could wrongly "correct". Wired into `buildGpx()` and `fetchElevationProfile()` (the same wrong-order bug was independently found in the elevation chart too - three ~300-600m fake "cliffs" at the exact segment boundaries, confirmed via the live `/api/v1/hikes/elevation` endpoint before fix, gone after)
- `utils/customHikes.js#parseGpx()`: now stitches at ingestion time (once, on upload) rather than leaving every reader to reorder on every request - also fixes a stored hike's own ascent/descent/elevation stats, which previously included the fake elevation "cliff" at each wrong segment boundary as if it were a real climb
- New exported `deriveStartEnd(stages)` (schweizMobilRoutes.js) - stitches within just the first/last stage (same multi-day-safety reasoning as above) and returns `{startPoint, endPoint}` as `{lat, lon}`, geometry-derived rather than from named GPX `<wpt>` waypoints (most uploads don't have any). Wired into both ASTRA route builders (`buildRoutesFromFeatures`, `fetchRouteStages`) and the custom-hike mapper (`hikeToTrailRoute`), so every hike - ASTRA and custom alike - now carries `startPoint`/`endPoint` in the merged `/api/v1/hikes` API response. Verified directly against this hike's real (still-unstitched) stored geometry - produces the correct true start/end coordinates
